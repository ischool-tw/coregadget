...
_gg.loadData = function () {
    ...
    _gg.connection.send({
        ...
        result: function (response, error, http) {
            if (error !== null) {
                _gg.set_error_message('#mainMsg', 'GetSession', error);
            } else {
                var _ref;
                if (((_ref = response.Session) != null ? _ref.Name : void 0) != null) {
                    $('#opening h3').html(response.Session.Name || '');
                } else {
