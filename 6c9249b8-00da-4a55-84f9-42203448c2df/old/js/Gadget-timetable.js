...
var MyViewDate = funStr2Date('2011-12-23 00:00:00'); //預設調課
var tabsNowDate = {};

var _connection = gadget.getContract("ischool.sunset.teacher"); //非同步
...
var Schedule_init = function () {
	var myCategory = {
		item: {
			categoryid: "T",
			myid: GadgetTimeTable.userInfo.TeacherID
		}
	};

	DoGetSchedule(myCategory, "myself");
};



function DoGetScheduledSchoolYearSemester() {
	_connection.send({
		service: "_.GetScheduledSchoolYearSemester",
		body: '',
		result: function (response, error, http) {
			// console.log(response);
			GadgetTimeTable.userInfo = _connection.getUserInfo();
			if (GadgetTimeTable.userInfo && GadgetTimeTable.userInfo.Property) {
				for (var i = 0; i < GadgetTimeTable.userInfo.Property.length; i++) {
					GadgetTimeTable.userInfo[GadgetTimeTable.userInfo.Property[i].Name] = GadgetTimeTable.userInfo.Property[i]['@text'];
				}
			}
...
//			$(response.SchoolYearSemester).each(function (index, item) {
//				var ScheduledYS = item.split(',');
//				SchoolYear = ScheduledYS[0];
//				Semester = ScheduledYS[1];
//				SchoolYear = "100";
//				Semester = "4";
...
//				Schedule_init();
//			});
			//console.log('DoGetScheduledSchoolYearSemester', new Date());
		}
	});
}
...
function DrawingMyTabs() {
	if (MySelf && MyTimeTable && MyCourseSection && colMySubstituteSchedule && colSrcMyExchangeCourse && colDesMyExchangeCourse) {
		...
		for (var TTID in MySelf) {
			...
			DrawingTimeTable("My" + TTID); //ToDo: 學期課表有多個timetable時不能這樣寫
		}
	}
}
...
function funMoveToWeek(setDate, CSID) {
	...
	tabsNowDate[MySelfTTID] = funStr2Date(setDate); //ToDo: 學期課表有多個timetable時不能這樣寫
