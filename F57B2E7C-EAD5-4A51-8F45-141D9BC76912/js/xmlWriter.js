...
(function(){
    ...
    XMLWriter.prototype = {
        ...
        writeAttributeString: function(name, value){
            if (this.active) 
                this.active.a[name] = "" + FormatXML(value).replace(/\n/g,'');
        },
        //add a text node to the active node
        writeString: function(text){
            if (this.active) 
                this.active.c.push("" + FormatXML(text));
        },
        ...
        flush: function(showHeader){
            if (this.stack && this.stack[0])//ensure it's closed
                this.writeEndDocument();
            var chr = '', indent = '', num = this.indentation, formatting = this.formatting.toLowerCase() == 'indented', buffer = "";
            if (showHeader) {
 ...
 : function(){// Mozilla, Firefox, Opera, etc.
    ...
    function format(node, indent, chr, buffer){
        var xml = indent + '<' + node.n, nc = node.c.length, attr, child, i = 0;
        ...
        if (nc) {
            do {
                ...
                if (typeof child == 'string') {
                    ...
                    if (typeof child == 'object') //element node
