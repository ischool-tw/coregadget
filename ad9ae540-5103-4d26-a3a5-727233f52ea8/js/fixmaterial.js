// 瀏覽我的單一教案
$(function() {
    LessonPlanManager.ControlFixMaterial = function(target) {
        var target = $(target)
            , _uid;

        target.find('[data-action=edit]').click(function() {
            LessonPlanManager.ControlAddmaterial.loadMaterial(_uid);
            $('<a href="#addmaterial" data-toggle="tab"></a>').tab('show');
        });

        target.find('[data-action=del]').click(function() {
            $('#delModal').modal("show");
        });

        $('#delModal').find('[data-action=del]').click(function() {
            delFixMaterial(_uid);
        });

        //#region 取得教材內容
        var getFixMaterial = function(uid, callback) {
        ...
        return {
            loadMaterial: function(uid) {
                ...
                target.find('[data-type=Content]').remove();
                _uid = uid;
                getFixMaterial(uid, showFixMaterial);
            }
        };
    }('#fixmaterial');
});
