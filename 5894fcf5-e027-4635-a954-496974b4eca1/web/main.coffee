	...
	gadget.getContract("ischool.course_selection.public").send {
		...
		result: (response, error, xhr) ->
			...
			gadget.getContract("ischool.course_selection.teacher").send {
				...
				result: (response, error, xhr) ->
					if response.Response.DeptName isnt ""
						getDeptSelectionResult()
					else if response.Response.ClassName isnt ""
						getClassSelectionResult()
					else
						alert "您並非為'科主任'或'班導師'，無法查詢學生選課結果!"
			}
	}

getDeptSelectionResult = () ->
	gadget.getContract("ischool.course_selection.teacher").send {
		service: "default.GetDeptSelectionResult",
		body: """
			<Request>
