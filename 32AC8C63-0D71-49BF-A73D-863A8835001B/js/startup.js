    ...
    cnCommon.send({
        ...
        result: function (resp, errorInfo, XMLHttpRequest) {
            if (errorInfo) {
                ...
                if (resp.Setting.Message) {
                    var divMsg = $('#divMsg');
                    divMsg.html('<button type="button" class="close">×</button>' + resp.Setting.Message);
                    $(divMsg.find('button')[0]).click(function () {
                        divMsg.addClass('hidden');
                        $($('#showMsg').removeClass('hidden').find('a')).click(function () {
