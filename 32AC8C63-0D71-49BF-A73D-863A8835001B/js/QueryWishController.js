...
ischool.chooseWish.QueryWishController = function() {
	...
	/* 取得指定班級的學生的志願清單 */ 
	var _getStudentWishes = function(classID) {
		if (!cnCommon)  
			cnCommon = ischool.chooseWish.cnCommon ;
		var reqdoc = '<Request>' +
	...
	var _fillBySeatNo = function() {
		...
		$(aryStudIDList).each(function(index, studID) {
            ...
            $(dicWishesByStudID[studID]).each(function(index, wish) {
            	...
            	var isLocked = (wish.FinalDeptCode == code);	//這位學生是否已經分發過了。
            	var html = '<tr class="stud_' + studID + ' ' + (isLocked ? "error" : "") + '"> \
                          	  <td dept-code="' + code + '">' + deptText + '</td> \
                          	  <td class="align-center">' + wish.WishPriority + '</td> \
                        	</tr>';
                var trWish = $(html);
                $(trWish.find('td')[0]).click(function() {
                	var deptCode = $(this).attr('dept-code');
                	$('#selDepts').val(deptCode);
                	_initStudentsByDeptCode(deptCode);
                });
			...
			var rankHtml = '<tr class="my-title"> \
	       	...
	       	$(studIDs).each(function(index, studID) {
	            ...
	            $(dicStuds[studID]).each(function(index, wish) {
	            	...
	            	var deptText = code + " - " + dept.School + " - " + dept.Dept + " (" + dept.Group + ")";
	            	var isLocked = (wish.FinalDeptCode == code);	//這位學生是否已經分發過了。
	            	var html = '<tr class="stud_' + studID + ' rank_' + rank + ' wish ' +  (isLocked ? "error" : "")  + '"> \
	                          	  <td dept-code="' + code + '">' + deptText + '</td> \
	                          	  <td class="align-center">' + wish.WishPriority + '</td> \
	                        	</tr>';
	                var trWish = $(html);
			...
			var rankHtml = '<tr class="my-title"> \
	       	...
	       	$(studIDs).each(function(index, studID) {
	            ...
	            $(dicStuds[studID]).each(function(index, wish) {
	            	...
	            	var deptText = code + " - " + dept.School + " - " + dept.Dept + " (" + dept.Group + ")";
	            	var isLocked = (wish.FinalDeptCode == code);	//這位學生是否已經分發過了。
	            	var html = '<tr class="stud_' + studID + ' rank_' + grp + ' wish ' + (isLocked ? "error" : "")   + '"> \
	                          	  <td dept-code="' + code + '">' + deptText + '</td> \
	                          	  <td class="align-center">' + wish.WishPriority + '</td> \
	                        	</tr>';
	                var trWish = $(html);
	...
	var _initStudentsByDeptCode = function(deptCode) {
				...
				if (errorInfo) {
					...
					$(studs).each(function(index, studWish) {
						html = ('<tr stud-id="' + studWish.StudentID + '" class-name="' + studWish.ClassName + '" seat-no="' + studWish.SeatNo + '" stud-name="' + studWish.StudentName +'"> \
	                                	<td><a href="#" cls-id="' + studWish.ClassID + '">' + studWish.ClassName + '</a></td> \
	                                	<td><a href="#" stud-id="' + studWish.StudentID + '">' + studWish.StudentName + '</a></td> \
