var _gg = _gg || {};
_gg.connection = gadget.getContract("ischool.service_learning.parent");
        ...
        result: function (response, error, http) {
            if (error !== null) {
                ...
                if (((_ref = response.Result) != null ? _ref.Student : void 0) != null) {
                    ...
                    // TODO: 無親子資料
                    $('select[name=semester], table[data-type=total], table[data-type=detail]').remove();
                    $('h1').after('<p>目前無資料</p>');
                }
            }
        }
...
_gg.GetSemester = function() {
    ...
    if (child.StudentID) {
        _gg.connection.send({
            ...
            body: '<Request><Condition><MyStudentID>' + child.StudentID + '</MyStudentID></Condition></Request>',
            result: function (response, error, http) {
                if (error !== null) {
...
_gg.GetStudentDetail = function() {
    ...
    if (child.StudentID && _gg.list_schoolyear && _gg.list_semester) {
        _gg.connection.send({
            service: "_.GetStudentDetail",
            body: '<Request><Condition>' +
                '<MyStudentID>' + child.StudentID + '</MyStudentID>' +
